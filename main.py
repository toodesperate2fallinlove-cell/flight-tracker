...
@app.route("/")
def home():
return """ <h1>Delhi Flight Tracker ✈️</h1>
...
```
if not q:
    return jsonify({"error": "Please provide q parameter"}), 400
...
html = f"<h1>Cheapest Flights From Delhi ({date})</h1>"
...
html = f"<h1>{origin.upper()} → {destination.upper()}</h1>"
